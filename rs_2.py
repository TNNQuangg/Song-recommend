...
def train_valid_test_split_by_user(data, valid_ratio=0.1, test_ratio=0.2, random_state=42):
    ...
    is_valid = is_valid_user & (~is_test) & (
        df["rank"] >= (user_counts - n_test - n_valid)
    )
...
def evaluate_model(model, matrix, K=10, mask_train=True):
    p = model.precision(matrix, K=K, mask_train=mask_train)
    r = model.recall(matrix, K=K, mask_train=mask_train)
    n = model.ndcg(matrix, K=K, mask_train=mask_train)
    h = model.hit_rate(matrix, K=K, mask_train=mask_train)
...
class Recommendation_System:
    ...
    def fit(self, verbose=True):
        ...
        for it in range(self.iterations):
            ...
            if verbose:
                print(f"  Iter {it+1:2d}/{self.iterations} completed")

    def _get_scores(self, u, mask_train=True):
        scores = self.X[u] @ self.Y.T

        if mask_train:
            train_items = self.R.indices[self.R.indptr[u]:self.R.indptr[u + 1]]
...
if __name__ == "__main__":
    ...
    play_count = pd.read_csv("dataSet/Cleaned_User_Listening_History.csv")
    track_df = pd.read_csv("dataSet/Music Info.csv")
    ...
    print("\n[INFO] Tuning hyperparameters trên Valid set...")
    print("[INFO] Chọn best model theo Valid NDCG@10\n")

    best_valid_ndcg = -1
    best_params = {}

    for k in [256]:
        for lamda in [0.2, 0.5, 2.0]:
            for alpha in [20, 40, 50]:
                ...
                valid_matrix = build_matrix(model, valid_data)
                valid_n = model.ndcg(valid_matrix, K=10, mask_train=True)

                print(
                    f"k={k}, lamda={lamda}, alpha={alpha} | "
                    f"Valid NDCG@10={valid_n*100:.2f}%"
                )

                if valid_n > best_valid_ndcg:
                    best_valid_ndcg = valid_n
                    best_params = {
                        "k": k,
                        "lamda": lamda,
                        "alpha": alpha
    ...
    print(f"\nBest params theo Valid NDCG@10: {best_params}")
    print(f"Best Valid NDCG@10: {best_valid_ndcg*100:.2f}%")

    train_valid_data = np.vstack([train_data, valid_data])

    final_model = Recommendation_System(
        train_valid_data,
        iterations=15,
        **best_params
    )
    ...
    test_p, test_r, test_n, test_hr = evaluate_model(
        final_model,
        test_matrix,
        K=10,
        mask_train=True
    ...
    print("\nSaved als12_model.pkl")

    user_test = train_data[0][0]
    track_test = train_data[0][1]

    print(f"\nRecommend cho user: {user_test}")
    print(final_model.recommend_user(user_test, track_df, top_k=10))

    print(f"\nTương tự track: {track_test}")
    print(final_model.recommend_track(track_test, track_df, top_n=10))